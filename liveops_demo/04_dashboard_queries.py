...
# MAGIC   FROM main.cursor_gaming.gold_kpi_5m GROUP BY bucket_5m
# MAGIC )
# MAGIC SELECT
# MAGIC   min(a.bucket_5m) AS window_start,
# MAGIC   max(a.bucket_5m) AS window_end,
# MAGIC   sum(a.impact_estimated) AS estimated_loss,
# MAGIC   (SELECT sum(dau_5m) FROM kpi k WHERE k.bucket_5m IN (SELECT bucket_5m FROM main.cursor_gaming.gold_anomaly WHERE is_anomaly = true)) AS affected_dau
